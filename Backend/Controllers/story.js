...
const addStory = asyncErrorWrapper(async  (req,res,next)=> {
    const { title, content, category } = req.body; // Extract category here
    ...
    try {
        const newStory = await Story.create({
            title,
            category, // Use the extracted category
            content,
            author: req.user._id,
...
// const searchByCategory = asyncErrorWrapper(async (req, res, next) => {
//     const { category } = req.query; // assuming category is sent as a query param
